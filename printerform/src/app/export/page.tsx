...
class PrintableFactory {
  ...
  public async initializeRenderEnvironment() {
    ...
    this.createLights(
      350,
      this.cameraOffsetsFromModelDimensions(this.modelDimensions)
    );
  ...
  public producePrintable(
    ...
    switch (projectionKind) {
      ...
      default:
        throw new Error(
          `Invalid projection kind provided: ${projectionKind}. Available options are ${Object.keys(
            ProjectionKind
          )}`
        );
    }
  ...
  public produceRevolvedPrintable(
    loadingCallback: (percentComplete: number) => any,
    onComplete: (pdfDataUrl: string) => any
  ) {
    ...
    templateImage.onload = () => {
      ...
      const doChunk = () => {
        ...
        while (
          ...
          const rendererContext = this.renderer.domElement.getContext(
            "webgl2",
          ...
          const sliceData = new ImageData(
            new Uint8ClampedArray(squareSlice),
            squareSize,
            squareSize
          );
      ...
      const renderSide = (
        ...
        if (rendererContext) {
          ...
          outputContext.drawImage(
            ...
            canvasWidth,
            canvasHeight / 2,
            (canvasWidth - modelWidth) / 2 + horizontalOffset,
            offsetY,
            modelWidth,
            modelHeight
          );
  ...
  private configureCamera(modelDimensions: ModelDimensions) {
    ...
    this.positionCamera(this.camera, modelDimensions, {
      phi: Math.PI / 2,
      theta: -Math.PI,
      radius: this.appropriateCameraDistance(modelDimensions),
    });
  ...
  private cameraOffsetsFromModelDimensions(
    modelDimensions: ModelDimensions
  ): [number, number, number] {
    const SCALE = 1;
  ...
  private configureRenderer(
    animationLoopCallback: XRFrameRequestCallback | null
  ) {
  ...
  private async loadModel(): Promise<ModelDimensions> {
    ...
    const material = new THREE.MeshStandardMaterial({
      color: this.modelInfo.modelColor,
      side: 2,
    });
...
const ALL_PROJECTIONS: ProjectionSelection[] = [
  ...
  },
  {
    ...
    projectionDescription:
      "Front and back of the model printed in a curved form",
...
const Export = () => {
  ...
  const [projectionSelections, setProjectionSelections] = useState<
    ProjectionSelection[]
  >([]);
  const [projectionLoadingInfo, setProjectionLoadingInfo] = useState<{
    remainingProjections: number;
    numTotalQueuedProjections: number;
  }>({ remainingProjections: 0, numTotalQueuedProjections: 0 });
  ...
  const runQueuedProjections = (
    printableFactory: PrintableFactory,
    queuedProjections: ProjectionSelection[]
  ) => {
    ...
    printableFactory.producePrintable(
      ...
      (generatedPdfUrl) => {
        ...
        setProjectionLoadingInfo((old) => ({
          ...old,
          remainingProjections: shrunkProjectionQueue.length,
        }));
  ...
  const handleProjectionDownload = async () => {
    ...
    const printableFactory = new PrintableFactory({
      modelUrl: modelUrl,
    ...
    <div className="flex flex-col h-screen w-full text-white">
      <div className="bg-[#1e1e1e] h-full overflow-scroll">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 pl-8 pr-8 pb-8">
          {ALL_PROJECTIONS.map((projectionSelection, ind) => {
            return (
              <div
                key={ind}
                className={`rounded-md overflow-hidden ${
                  projectionSelections.find(
                    (selection) => selection.key === projectionSelection.key
                  )
                    ? "border-blue-600"
                    : "border-[#2c2c2c] hover:border-blue-300"
                } transition duration-200 border-2 h-96`}
                onClick={() => {
                  if (
                    projectionSelections.find(
                      (selection) => selection.key === projectionSelection.key
                    ...
                    setProjectionSelections(
                      projectionSelections.filter(
                        (selection) => selection.key !== projectionSelection.key
                      )
                    );
                  } else {
                    setProjectionSelections((old) => [
                      ...old,
                      projectionSelection,
                    ]);
                  }
              ...
              >
                <img
                  src={projectionSelection.previewThumbnailImgSrc}
                  className="h-52 w-full"
                />

                <div className="w-full h-full bg-[#222222] p-4 flex flex-col gap-y-2">
                  <h2 className="text-xl font-bold">
                    {projectionSelection.projectionTitle}
                  </h2>
        ...
        <div className="bg-[#1e1e1e] px-5 py-3 flex flex-row justify-between items-center">
          ...
          <button
            type="button"
            className={`${
              projectionSelections.length === 0 || isLoading
                ? "bg-gray-300 text-gray-500 cursor-not-allowed"
                : "bg-blue-500"
            } transition duration-200 rounded-md p-5`}
            onClick={
              projectionSelections.length === 0 || isLoading
                ? undefined
                : handleNext
        ...
        <div className="bg-[#2c2c2c] px-5 py-3 flex flex-row justify-between items-center">
          {projectionLoadingInfo.remainingProjections === 0 ? (
            ...
            <div className="flex flex-row items-center gap-x-4">
              <p className="text-xl">
                Producing Projection Printable{" "}
                {projectionLoadingInfo.numTotalQueuedProjections -
                  projectionLoadingInfo.remainingProjections +
                  1}
                /{projectionLoadingInfo.numTotalQueuedProjections}
              </p>
              <p>|</p>
