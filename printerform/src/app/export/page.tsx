...
class PrintableFactory {
   ...
   public producePrintable(projectionKind: ProjectionKind, loadingCallback: (percentComplete: number) => any, onComplete: (pdfDataUrl: string) => any) {
       switch (projectionKind) {
           ...
           case ProjectionKind.CURVED:
               ...
               return;
           case ProjectionKind.CURVED_SIDE_VIEW:
                   this.produceCurvedSideViewPrintable(loadingCallback, onComplete);
                   return;
           default:
   ...
   public produceCurvedPrintable(loadingCallback: (percentComplete: number) => any, onComplete: (pdfDataUrl: string) => any) {
       ...
       img.onload = () => {
           ...
           const renderSide = (cameraAngle: number, flipHorizontally: boolean, flipVertically: boolean, offsetY: number, horizontalOffset: number = 0, scale: number
               ...
               this.positionCamera(this.camera, this.modelDimensions, {
                   phi: cameraAngle, theta: 0, radius: this.appropriateCameraDistance(this.modelDimensions)
               });
           ...
           // top model
           renderSide(0, true, false, 80, 80, 0.9);
           // bottom model
           renderSide(Math.PI, false, true, 80, 80, 0.9);
   ...
   public produceCurvedSideViewPrintable(loadingCallback: (percentComplete: number) => any, onComplete: (pdfDataUrl: string) => any) {
       const img = new Image();
       img.src = curvedBackgroundImage;
       ...
       img.onload = () => {
           ...
           // top model
           renderSide(Math.PI/2, true, false, 80, 80, 0.9);
           // bottom model
           renderSide(3 * Math.PI / 2, false, true, 80, 80, 0.9);
...
const ALL_PROJECTIONS: ProjectionSelection[] = [
   ...
   // },
   {
       ...
       projectionDescription: "Displays the model on four perpendicular planes",
       projectionTitle: "Four Sided Cube Projection"
   },
   {
       key: ProjectionKind.SIX_SIDED_CUBE,
       previewThumbnailImgSrc: "six_side_2.jpg",
       projectionDescription: "Displays the model on all six sides of a cube",
       projectionTitle: "Six Sided Cube Projection"
   },
   {
       key: ProjectionKind.CURVED,
       previewThumbnailImgSrc: "curved_2.jpg",
       projectionDescription: "Displays the model's front and back on a smooth continuous curve",
       projectionTitle: "Curved Volume Projection"
   },
   {
       key: ProjectionKind.CURVED_SIDE_VIEW,
       previewThumbnailImgSrc: "curved_2.jpg",
       projectionDescription: "Displays the model's side views on a smooth continuous curve",
       projectionTitle: "Curved Volume Projection - Side View"
   }
...
const Export = () => {
   ...
   const downloadPdf = () => {
       ...
       link.download = 'form.pdf'; // change to handle other projections
       link.click();
   };
                   ...
                   <div className="bg-[#1e1e1e] px-5 py-3 flex flex-row justify-between items-center">
                       ...
                       <button
                           ...
                           className={`${projectionSelections.length === 0 || isLoading
                               ? "bg-light-gray-300 text-gray-500 cursor-not-allowed"
                               : "bg-light-gray-500 hover:bg-light-gray-300 text-white"} transition duration-200 rounded-md p-5`} // Added text-white here
                           onClick={projectionSelections.length === 0 || isLoading ? undefined : handleNext}
                           style={{ backgroundColor: projectionSelections.length === 0 || isLoading ? '#8E2929' : '#8E2929' }} // Retained the background color
                       >
                           Next
                       </button>
                   </div>
...
export default () => <Suspense><Export /></Suspense>;
